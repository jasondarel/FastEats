...
const getStatusColor = (status) => {
    ...
    return statusColors[status?.toUpperCase()] || '#f39c12';
};

const sendOrderPreparingEmail = async ({
    recipientType,
    toEmail,
    recipientName,
    order_id,
...
`;
  ...
  logger.info(`📧 Order preparing email sent to ${toEmail} (${recipientType}) for order #${order_id}`);
};
...
const sendOrderNotifications = async (payload) => {
  ...
  if (customerEmail) {
    tasks.push(sendOrderPreparingEmail({
...
const startOrderPreparingConsumer = async () => {
  try {
    ...
    setTimeout(startOrderPreparingConsumer, 5000);
  }
};

export default startOrderPreparingConsumer;