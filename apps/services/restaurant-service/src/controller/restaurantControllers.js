...
} from "../validator/restaurantValidators.js";
...
const GLOBAL_SERVICE_URL =
  process.env.GLOBAL_SERVICE_URL || "http://localhost:3000";
...
export const createRestaurantController = async (req, res) => {
  ...
  try {
    ...
    logger.info(
      `Create restaurant success: name: ${newRestaurant.restaurant_name}`
    );
...
export const updateRestaurantController = async (req, res) => {
  ...
  try {
    ...
    logger.info(
      `Restaurant updated successfully: name: ${updatedRestaurant.restaurant_name}`
    );
    return responseSuccess(
      res,
      200,
...
export const deleteRestaurantController = async (req, res) => {
  ...
  try {
    ...
    logger.info(
      `Delete restaurant success: name: ${deletedRestaurant.restaurant_name}`
    );
    return responseSuccess(
      res,
      200,
...
export const getRestaurantByOwnerIdController = async (req, res) => {
  ...
  try {
    ...
    return responseSuccess(
      res,
      200,
      "Get restaurant by owner id success",
      "restaurant",
      result
    );
  } catch (err) {
...
export const getRestaurantByRestaurantIdController = async (req, res) => {
  ...
  try {
    ...
    return responseSuccess(
      res,
      200,
      "Get restaurant by restaurant id success",
      "restaurant",
      result
    );
  } catch (err) {
  ...
  try {
    ...
    if (restaurant.owner_id !== userId) {
      ...
      return responseError(
        res,
        403,
        "You are not authorized to open this restaurant"
      );
    }

    const updatedRestaurant = await updateOpenRestaurantService(
      restaurantId,
      isOpen
    );
  ...
  try {
    ...
    if (order.order.status !== "Completed") {
      ...
      return responseError(
        res,
        400,
        "Order must be delivered to rate a restaurant"
      );
    }
    ...
    return responseSuccess(
      res,
      201,
      "Restaurant rating created successfully",
      "restaurantRating",
      restaurantRating
    );
  } catch (err) {
...
export const getRestaurantDetailRatingController = async (req, res) => {
  ...
  try {
      ...
      details.ratings = await Promise.all(
        ratings.map(async (rating) => {
          const user = await getUserInformation(
            ...
            `User with ID ${rating.user_id} not found`
          );
          const order = await getOrderInformation(
            GLOBAL_SERVICE_URL,
            rating.order_id,
            req.headers.authorization,
            `Order with ID ${rating.order_id} not found`
          );
          return {
            ...rating,
    ...
    return responseSuccess(
      res,
      200,
      "Get restaurant detail rating success",
      "data",
      details
    );
  } catch (err) {
